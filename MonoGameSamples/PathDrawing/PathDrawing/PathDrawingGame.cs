...
namespace PathDrawing
{
    ...
    public class PathDrawingGame : Microsoft.Xna.Framework.Game
    {
        ...
        private void HandleMouseInput()
        {
            ...
            {
                Vector2 position = currentMouseState.Position.ToVector2();
                if (tank.HitTest(position))
                {
                    tank.Waypoints.Clear();
                    drawingWaypoints = true;
                    tank.Waypoints.Enqueue(position);
                }
            }
            ...
            else if (drawingWaypoints)
            {
                Vector2 position = currentMouseState.Position.ToVector2();
                if (position != previousMouseState.Position.ToVector2())
                    tank.Waypoints.Enqueue(position);
            }
