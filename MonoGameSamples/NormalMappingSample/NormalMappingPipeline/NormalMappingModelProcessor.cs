    ...
    public class NormalMappingModelProcessor : ModelProcessor
    {
        ...
        private void LookUpNormalMapAndAddToTextures(NodeContent node)
        {
            ...
            if (mesh != null)
            {
                ...
                {
                    // The original XNA tutorial was using an FBX importer that could load this information out of
                    // opaque data. It appears AssImp can't do that, so we're making an alternative last-ditch attempt
                    // to guess where the normal map is, based on a loose convention (a.k.a., "this is where this one
                    // model happened to put it"). We're also assuming a TGA file. This is a *lot* of assumptions, and
                    // isn't ideal.
                    pathToNormalMap = mesh.Name.Substring(0, mesh.Name.LastIndexOf("_")) + "_norm.tga";
