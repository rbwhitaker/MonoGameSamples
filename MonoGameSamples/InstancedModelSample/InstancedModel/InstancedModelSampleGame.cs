...
namespace InstancedModelSample
{
    ...
    public class InstancedModelSampleGame : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            instancedModel.Meshes[0].MeshParts[0].Effect.Parameters["Texture"].SetValue(Content.Load<Texture2D>("cat"));
            instancedModel.Meshes[0].MeshParts[0].Effect.Parameters["DiffuseLight"].SetValue(new Vector3(1, 1, 1));
            instancedModel.Meshes[0].MeshParts[0].Effect.Parameters["AmbientLight"].SetValue(new Vector3(0.25f, 0.25f, 0.25f));
            instancedModel.Meshes[0].MeshParts[0].Effect.Parameters["LightDirection"].SetValue(Vector3.Normalize(new Vector3(1, 1, 1)));
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                foreach (ModelMeshPart meshPart in mesh.MeshParts)
                {
                    ...
                    foreach (EffectPass pass in effect.CurrentTechnique.Passes)
                    {
                        ...
                        GraphicsDevice.DrawInstancedPrimitives(PrimitiveType.TriangleList, 0, 0, meshPart.PrimitiveCount, instances.Length);
                    }
                }
            }
        }
